...
- **Reverse-design blind spot (resolved by Phase 5):** `passes_uncertainty` in `ReverseV3Designer` is based on cross-seed ensemble disagreement, which does not catch the low-drag/Mach>=0.49 failure mode above, since all seeds are consistently biased the same direction rather than disagreeing. This was flagged as needing an explicit guardrail before frontend use -- Phase 5's `mach_warning` system (extrapolation distance to the known Mach grid, surfaced as a badge in the UI) addresses the practical version of this for Mach specifically. Note it's a narrower fix than the general problem: it catches "querying somewhere the model has no training support" but wouldn't catch a hypothetical future case where the model is confidently wrong *within* a well-supported region for some other reason. Worth remembering that distinction if this comes up again for a different target/regime.